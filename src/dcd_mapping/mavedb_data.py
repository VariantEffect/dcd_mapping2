...
def get_scoreset_records(
    ...
    if not scores_csv.exists():
        ...
        try:
            ...
            _logger.info("Attempting paginated HTTP download for %s", scoreset_urn)
            try:
                paginated_http_download(url, scores_csv, silent)
            except requests.HTTPError as e:
                msg = f"Paginated download for {scoreset_urn} failed"
                _logger.error(msg)
                raise ResourceAcquisitionError(msg) from e
        except requests.HTTPError as e:
